...
app.controller('inicioCtrl', function ($scope, $rootScope, $timeout, LoginService, services, $location, configuracion) {
    ...
    services.tieneVoto($scope.id).then(function (data) {
...
app.controller('votarCtrl', function ($scope, services, LoginService, $location) {
    // var ncomision = LoginService.comision();
    var ncomision = localStorage.getItem('comision');

    $scope.ncomision = ncomision;

    services.getPostulantes(ncomision).then(function (data) {
    ...
    $scope.registrarVoto = function (idAlumno, idPostulante, alumno) {
        ...
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    services.insertarVoto(idAlumno, idPostulante).then(function (data) {
                        console.log(data.data);
                        $location.path('/inicio');
                    });
                }
...
app.controller('estadisticasCtrl', function ($scope, services, LoginService) {
    // $scope.role = LoginService.role();
    ...
    angular.element(document).ready(function () {
        if ($scope.role == 0) {
            ...
            $scope.cambiarComision($scope.comision, $scope.distrito);
        }

        $('#comisiones').on('change', function () {
            var ncomision = $("#comisiones option:selected").text();
            $scope.cambiarComision(ncomision);
        })
    })

    $scope.cambiarComision = function (ncomision, distrito) {
        console.log('ncomision, distrito: ' + ncomision, distrito)
        services.getTotal(ncomision, distrito).then(function (data) {
            $scope.result = data.data;
        })

        services.getEstadisticas(ncomision, distrito).then(function (data) {
