<?php
	...
	private function ver_postulantes()
	{
		...
		$query = "SELECT CONCAT(apellido,', ', nombre) as alumno, dni, id as id_postulante
							FROM `fichas` 
							WHERE es_postulante = 1 AND comision = $ncomision";
	...
	private function insertar_voto()
	{
		...
		$voto = (int)$this->_request['idvoto'];

		if (!empty($idAlumno) && !empty($voto)) {
		$query = "INSERT INTO votos (id_alumno, voto) VALUES ($idAlumno, $voto)";
	...
	private function estadisticas()
	{
		...
		$query = "SELECT CONCAT(f.apellido,', ', f.nombre) as alumno, COUNT(v.voto)  as votos
							...
							ON f.id=v.voto
							WHERE rol=0 AND f.comision = $ncomision and f.distrito=$distrito
							GROUP BY v.voto";
